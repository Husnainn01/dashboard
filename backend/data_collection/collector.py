...
class DataCollector:
    ...
    async def stream_realtime_ticks(self, asset: str, stop_event: asyncio.Event = None):
        """
        Async generator that yields real-time price ticks for an asset.
        Polls realtime_price every ~1 second.
        Yields {"asset": str, "price": float, "timestamp": float} when price changes.
        Re-subscribes automatically if no data for 10 seconds.
        """
        if not self.is_connected:
            self.logger.error("❌ Not connected to PyQuotex — cannot stream ticks")
            return

        self.logger.info(f"📡 Subscribing to realtime candle for {asset}...")
        try:
            await self.client.subscribe_realtime_candle(asset, self.timeframe)
        except Exception as e:
            self.logger.error(f"❌ Failed to subscribe to realtime candle for {asset}: {e}")
            return
        ...
        while True:
            ...
            if stop_event and stop_event.is_set():
                self.logger.info(f"🛑 Stop event set — ending tick stream for {asset}")
                return
            ...
            try:
                ...
                if realtime_data:
                    last_data_time = tick_time
                    if isinstance(realtime_data, list) and len(realtime_data) > 0:
                        entry = realtime_data[-1]
                        if isinstance(entry, dict):
                            price = float(entry.get('close', entry.get('price', 0)))
                        elif isinstance(entry, (int, float)):
                            price = float(entry)
                    elif isinstance(realtime_data, dict):
                        price = float(realtime_data.get('close', realtime_data.get('price', 0)))
                    elif isinstance(realtime_data, (int, float)):
                        price = float(realtime_data)

                # Re-subscribe if no data for 10 seconds
                if time.time() - last_data_time > 10:
                    self.logger.warning(f"⚠️ No realtime data for {asset} in 10s — re-subscribing...")
                    try:
                        await self.client.subscribe_realtime_candle(asset, self.timeframe)
                    except Exception as sub_err:
                        self.logger.error(f"❌ Re-subscribe failed for {asset}: {sub_err}")
                    last_data_time = time.time()

                # Yield only when price is valid and changed
                if price and price > 0 and price != last_price:
                    last_price = price
                    yield {
                        "asset": asset,
                        "price": price,
                ...
                self.logger.error(f"❌ Tick stream error for {asset}: {e}")
                # Don't crash the generator — let caller handle retry
                return
